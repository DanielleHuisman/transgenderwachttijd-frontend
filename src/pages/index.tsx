...
import {
    ...
    type GetProvidersQueryVariables,
    GetServiceAgeGroupsDocument,
    type GetServiceAgeGroupsQuery,
    type GetServiceAgeGroupsQueryVariables,
    GetServiceTreeDocument,
    type GetServiceTreeQuery,
    type GetServiceTreeQueryVariables,
    GetServiceTypesDocument,
    type GetServiceTypesQuery,
    type GetServiceTypesQueryVariables,
    type ProviderFragment,
    type ServiceAgeGroupFragment,
    type ServiceFragment,
    type ServiceTypeFragment
} from '../graphql/generated';
import {type FlatTreeItem, flattenTree} from '../util/tree';
...
export const getServerSideProps: GetServerSideProps<Props> = async ({locale}) => {
    const {
        data: {providers}
    } = await client.query<GetProvidersQuery, GetProvidersQueryVariables>({
        query: GetProvidersDocument
    });
    const {
        data: {serviceTree}
    } = await client.query<GetServiceTreeQuery, GetServiceTreeQueryVariables>({
        query: GetServiceTreeDocument
    });
    const {
        data: {serviceAgeGroups}
    } = await client.query<GetServiceAgeGroupsQuery, GetServiceAgeGroupsQueryVariables>({
        query: GetServiceAgeGroupsDocument
    });
    const {
        data: {serviceTypes}
    } = await client.query<GetServiceTypesQuery, GetServiceTypesQueryVariables>({
...
const Home: NextPage<Props> = ({providers, services, serviceAgeGroups, serviceTypes}) => {
    ...
    const [serviceIds, setServiceIds] = useState<string[]>(
        services.filter(([service]) => ['Psychology', 'Intake'].includes(service.name)).map(([service]) => service.id)
    );
    const [serviceAgeGroupIds, setServiceAgeGroupIds] = useState<string[]>(
        serviceAgeGroups.map((serviceAgeGroup) => serviceAgeGroup.id)
    );
    ...
    return (
        <Container>
            <p className="mb-4">
                This website is still work in progress. More information will be added in the future. See the{' '}
                <a href="https://github.com/DanielHuisman/transgenderwachttijd-frontend">GitHub repository</a> for
                progress.
            </p>
            ...
            <div className="d-flex gap-3 mb-3">
                ...
                <ServiceDropdown services={services} value={serviceIds} onChange={(value) => setServiceIds(value)} />
